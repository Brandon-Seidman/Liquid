...
const Post = (props) => {
  ...
  async function HandleSubmit(event) {
    try {
      let user = (await axios.get(`http://localhost:443/users/${cookies.get("userId")}`)).data;
      let result = await axios.post(`http://localhost:443/posts/comment/${props.match.params.id}`, {
  ...
  useEffect(() => {
    ...
    async function getPostData() {
      ...
      let newData = await axios.get(
        "http://localhost:443/posts/" + props.match.params.id
      );
      let likeData = await axios.post("http://localhost:443/posts/isLiked", {
                  ...
                  {liked && (
                    <FavoriteIcon
                      onClick={async (event) => {
                        ...
                        await axios.post("http://localhost:443/posts/unlike", {
                  ...
                  {!liked && (
                    <FavoriteBorderIcon
                      onClick={async (event) => {
                        ...
                        dispatch(actions.setLikes(likes+1));
                        await axios.post("http://localhost:443/posts/like", {
                          username: data.data.user,
                          userId: cookies.get("userId"),
                          postId: props.match.params.id,
                        });
